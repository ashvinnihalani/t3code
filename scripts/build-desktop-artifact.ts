...
export const WINDOWS_ASAR_UNPACK = ["**/node_modules/**"] as const;
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    // Keep Windows native dependencies outside the asar so Electron can load them.
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const bundledClientEntry = path.join(distDirs.webDist, "index.html");

  if (!options.skipBuild) {
    yield* Effect.log("[desktop-artifact] Building desktop and web artifacts...");
  ...
  const requiredBuildInputs = [
    ...
    { artifact: "web-dist", artifactPath: distDirs.webDist },
  ] as const;
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    description: "T3 Codex desktop control harness",
    author: "T3 Tools",
