...
T3 Codex is a desktop-oriented fork of T3 Code for controlling Codex app-server-compatible agents.
...
**Desktop** is the target product surface. It is an Electron app that wraps the web renderer and is
being migrated from the bundled T3 server to a generic Codex app-server transport.
...
- **client** means the web renderer or desktop shell.
...
- **Clients.** Web and desktop (which wraps web and adds Electron shell/IPC). Shared logic lives in `packages/client-runtime`.
...
- Upon request, user-visible frontend changes should get one integrated pass with `test-t3-app`. Ask permission before doing computer use or spinning up browsers.
...
- `apps/web` - React/Vite UI. `apps/desktop` wraps it with the Electron shell.
- `packages/contracts` - Effect/Schema contracts plus small derived helpers. No heavy runtime logic.
- `packages/shared` - shared runtime utils, subpath exports, no barrel.
- `packages/client-runtime` - non-visual client code used by the web renderer.
