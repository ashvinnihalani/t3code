...
function resolveWsRpc(body: WsRequestEnvelope["body"]): unknown {
  ...
  if (tag === WS_METHODS.terminalOpen) {
    return {
      ...
      terminalId:
        typeof body.terminalId === "string" ? body.terminalId : "default",
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("does not clear local Codex errors on initial settings hydration but clears them after a later settings change", async () => {
    ...
    try {
      ...
      dispatchAppSettingsChange(
        buildAppSettings({ codexBinaryPath: "/usr/local/bin/codex" }),
      );

      await vi.waitFor(() => {
        expect(
          useStore.getState().localCodexErrorsDismissedAfter,
        ).not.toBeNull();
  ...
  it("prepares a worktree when a remote draft is in worktree mode", async () => {
    ...
    try {
      useComposerDraftStore
        .getState()
        .setPrompt(THREAD_ID, "remote draft send");
      await vi.waitFor(
        () => {
      ...
      const createdWorktreeBranch =
        typeof createWorktreeRequest?.newBranch === "string"
  ...
  it("uses the draft title override when creating a new thread", async () => {
    ...
    try {
      ...
      useComposerDraftStore
        ...
        .setPrompt(
          newThreadId,
          "Prepare the release checklist and rollout plan.",
        );
  ...
  it("resets the draft title override after navigating away from the new thread", async () => {
    ...
    try {
      ...
      expect(
        document.querySelector('[data-testid="draft-thread-title-input"]'),
      ).toBeNull();
  ...
  it("creates a fresh draft after the previous draft thread is promoted", async () => {
    ...
    try {
      ...
      const { syncServerReadModel } = useStore.getState();
      syncServerReadModel(
        addThreadToSnapshot(fixture.snapshot, promotedThreadId),
      );
