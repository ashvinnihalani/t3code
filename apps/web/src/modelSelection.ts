...
export type CustomModelSettings = {
  ...
  customKiroModels: readonly string[];
};
...
export function getCustomModelOptionsByProvider(
  ...
  return {
    ...
    kiro: getAppModelOptions(
      "kiro",
      customModelsByProvider.kiro,
      selectedProvider === "kiro" ? selectedModel : undefined,
    ),
  };
}

function isGitTextGenerationProvider(
  provider: ProviderKind,
): provider is keyof typeof DEFAULT_GIT_TEXT_GENERATION_MODEL_BY_PROVIDER {
...
export function resolveAppModelSelectionState(
  ...
  const rawSelection = settings.textGenerationModelSelection ?? fallbackSelection;
  const provider = isGitTextGenerationProvider(rawSelection.provider)
    ? rawSelection.provider
    : fallbackSelection.provider;
  const customModelsByProvider = getCustomModelsByProvider(settings);
  const model = resolveAppModelSelection(
    provider,
    customModelsByProvider,
    rawSelection.provider === provider ? rawSelection.model : fallbackSelection.model,
  );
  const { modelOptionsForDispatch } = getComposerProviderState({
    ...
    prompt: "",
    modelOptions: {
      [provider]: rawSelection.provider === provider ? rawSelection.options : undefined,
    },
  });
