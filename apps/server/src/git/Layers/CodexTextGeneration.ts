...
function withAdditionalSystemInstructions(
  sections: string[],
  systemPrompt?: string | null,
): string[] {
  const customSystemPrompt = systemPrompt?.trim() ?? "";
  const [lead, ...rest] = sections;
  if (!lead || customSystemPrompt.length === 0) {
    return sections;
  }

  return [
    lead,
    "Follow these additional system instructions exactly:",
    customSystemPrompt,
    "",
    ...rest,
  ];
...
const makeCodexTextGeneration = Effect.gen(function* () {
  ...
  const generateCommitMessage: TextGenerationShape["generateCommitMessage"] = (input) => {
    ...
    const prompt = withAdditionalSystemInstructions(
      [
  ...
  const generateBranchName: TextGenerationShape["generateBranchName"] = (input) => {
    return Effect.gen(function* () {
      ...
      const promptSections = withAdditionalSystemInstructions(
        [
