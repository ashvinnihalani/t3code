...
export const makeGitCore = (options?: { executeOverride?: GitCoreShape["execute"] }) =>
  Effect.gen(function* () {
        ...
        if (remote?.kind === "ssh") {
            ...
            try: () =>
              runProcess(
                ...
                }),
                {
                  ...
                  outputMode: truncateOutputAtMaxBytes ? "truncate" : "error",
                },
              ),
          ...
          return {
            ...
            stderr: result.stderrTruncated
              ? `${result.stderr}${OUTPUT_TRUNCATED_MARKER}`
              : result.stderr,
    ...
    const runGitStdoutWithOptions = (
      ...
      executeGit(operation, cwd, args, options, remote).pipe(Effect.map((result) => result.stdout));
      ...
      lookup: (cacheKey: StatusUpstreamRefreshCacheKey) =>
        Effect.gen(function* () {
          yield* fetchUpstreamRefForStatus(
            ...
            cacheKey.remoteHostAlias ? { kind: "ssh", hostAlias: cacheKey.remoteHostAlias } : null,
          );
    ...
    const commit: GitCoreShape["commit"] = (cwd, subject, body, options) =>
      Effect.gen(function* () {
